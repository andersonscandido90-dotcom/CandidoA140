...
const StabilityPanel: React.FC<Props> = ({ data, fuelData, onChange }) => {
  ...
  const hydrostaticTable = useMemo(() => [
    ...
    // LINHA 6.5m - VALORES CORRETOS DO EXEMPLO
    { draft: 6.5, nivel: 20777.6, deProa: 20358.4, deRé: 21230.5 }, // ← CORRETO!
    { draft: 6.4, nivel: 20358.4, deProa: 19941.4, deRé: 20806.8 },
    { draft: 6.3, nivel: 19941.4, deProa: 19526.5, deRé: 20385.2 },
    { draft: 6.2, nivel: 19526.5, deProa: 19113.8, deRé: 19965.9 },
    { draft: 6.1, nivel: 19113.8, deProa: 18703.2, deRé: 19548.6 },
    { draft: 6.0, nivel: 18703.2, deProa: 18294.9, deRé: 19133.5 },
    { draft: 5.9, nivel: 18294.9, deProa: 17888.9, deRé: 18720.6 },
    { draft: 5.8, nivel: 17888.9, deProa: 17485.2, deRé: 18309.8 },
    { draft: 5.7, nivel: 17485.2, deProa: 17084.0, deRé: 17901.3 },
    { draft: 5.6, nivel: 17084.0, deProa: 16685.3, deRé: 17494.9 },
    { draft: 5.5, nivel: 16685.3, deProa: 16289.3, deRé: 17090.8 },
    { draft: 5.4, nivel: 16289.3, deProa: 15896.1, deRé: 16689.0 },
    { draft: 5.3, nivel: 15896.1, deProa: 15506.0, deRé: 16289.5 },
    { draft: 5.2, nivel: 15506.0, deProa: 15119.1, deRé: 15892.4 },
    { draft: 5.1, nivel: 15119.1, deProa: 14735.3, deRé: 15497.8 },
    { draft: 5.0, nivel: 14735.3, deProa: 14354.9, deRé: 15105.7 },
  ], []);

  // === Lógica Hidrostática CORRIGIDA ===
  const hydrostatics = useMemo(() => {
    console.log('=== NOVO CÁLCULO ===');
    console.log('AV:', data.draftForward, 'AR:', data.draftAft);
    ...
    // 1. Encontrar valores na tabela para interpolação
    const sortedTable = [...hydrostaticTable].sort((a, b) => a.draft - b.draft);
    
    // Encontrar lower (calado ≤ meanDraft) e upper (calado ≥ meanDraft)
    ...
    console.log('Tabela: lower', lower.draft, 'upper', upper.draft);
    
    // 2. Determinar qual coluna usar baseado no trim
    const trimAbs = Math.abs(trim);
    let column: keyof typeof lower = 'nivel'; // padrão = trim ≤ 1.0m
    
    if (trimAbs > 1.0) {
      column = trim > 0 ? 'deProa' : 'deRé';
    }
    
    console.log('Coluna usada:', column, '(trimAbs:', trimAbs, ')');
    
    // 3. Interpolação linear para K (valor da tabela para o trim atual)
    ...
    console.log('Valores:');
    console.log('K (tabela, trim', trim.toFixed(2), '):', K.toFixed(1));
    console.log('C (nível, trim 0):', C.toFixed(1));
    ...
    console.log('GM = (V × t) / u =', gm.toFixed(4));
    
    // 7. Calcular KM aproximado
  ...
  useEffect(() => {
    if (Math.abs(hydrostatics.gm - data.gm) > 0.001) {
      console.log('Atualizando GM no estado:', hydrostatics.gm.toFixed(4));
  ...
  return (
    <div className="bg-slate-900/80 border border-slate-800 shadow-2xl rounded-[1.5rem] sm:rounded-2xl lg:rounded-[3rem] p-5 sm:p-6 lg:p-10 flex flex-col gap-6 sm:gap-10 backdrop-blur-md">
      ...
      {/* Painel de status do GM com detalhes */}
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex flex-col items-end">
            ...
            <span className="text-xs text-slate-600 mt-1">
              C: {hydrostatics.C_value?.toFixed(1) || '0.0'} t | K: {hydrostatics.K_value?.toFixed(1) || '0.0'} t
            </span>
          </div>
        </div>
      </div>
