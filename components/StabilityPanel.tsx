...
const StabilityPanel: React.FC<Props> = ({ data, onChange }) => {
  ...
  // CINEMÁTICA VISUAL:
  // Alinhamento da linha d'água (y=85 no SVG) com a referência visual
  const rotationAngle = -heightDiff * 3.0; 
  const sensitivity = 18; 
  // O ponto neutro é 7.0m. O verticalOffset move o navio para cima/baixo conforme o calado.
  const verticalOffset = -(meanDraft - 7.0) * sensitivity;

  // LÓGICA DE BANDA DUAL
  const bbValue = data.heel < 0 ? Math.abs(data.heel) : 0;
  const beValue = data.heel > 0 ? data.heel : 0;
  ...
  const renderInput = (label: string, value: number, key: keyof StabilityData, unit: string, icon: React.ReactNode, specialChange?: (v: number) => void) => (
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            onChange={(e) => {
              ...
              if (specialChange) specialChange(val);
    ...
    <div className="bg-slate-900/80 border border-slate-800 shadow-2xl rounded-[1.5rem] sm:rounded-[2rem] lg:rounded-[3rem] p-5 sm:p-6 lg:p-10 flex flex-col gap-6 sm:gap-10 backdrop-blur-md">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
        ...
        <div className="bg-slate-950/50 border-2 border-slate-800 rounded-[1.5rem] sm:rounded-[2.5rem] p-4 sm:p-8 flex flex-col items-center min-h-[350px] sm:min-h-[480px] relative overflow-hidden">
          <div className="w-full flex justify-between items-center mb-4 relative z-50">
            <span className="text-[9px] sm:text-[11px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
              <Ship size={14} className="sm:w-4 sm:h-4" /> TRIM
            </span>
            <div className={`px-3 py-1 sm:px-5 sm:py-2 rounded-lg sm:rounded-xl text-[8px] sm:text-[11px] font-black uppercase tracking-widest shadow-2xl border-2 transition-all duration-500 ${statusColor}`}>
              {statusLabel}
            </div>
          </div>
          
          <div className="flex-1 flex items-center justify-center relative w-full h-full overflow-hidden">
            {/* Linha de Referência da Água */}
            <div className="absolute inset-x-0 top-[60%] h-[2px] bg-blue-500/40 z-10 shadow-[0_0_15px_rgba(59,130,246,0.5)]" />
            
            {/* Textos de fundo removidos conforme solicitado */}

            <div 
              className="relative w-full transition-all duration-1000 cubic-bezier(0.2, 0.8, 0.2, 1) z-20 px-4"
              style={{ 
                transform: `translateY(${verticalOffset}px) rotate(${rotationAngle}deg)`,
                transformOrigin: 'center center' 
              }}
            >
              <svg viewBox="0 0 500 150" className="w-full h-auto drop-shadow-[0_15px_30px_rgba(0,0,0,0.8)] sm:drop-shadow-[0_30px_60px_rgba(0,0,0,0.8)]">
              ...
              <div className="absolute left-4 top-[85px] -translate-y-1/2 -translate-x-1/2 flex flex-col items-center">
                <span className="text-[8px] sm:text-[10px] font-black text-white uppercase bg-slate-900 px-2 py-0.5 sm:px-3 sm:py-1 rounded-full border border-slate-700 shadow-xl">AR</span>
              </div>
              <div className="absolute right-4 top-[85px] -translate-y-1/2 translate-x-1/2 flex flex-col items-center">
                <span className="text-[8px] sm:text-[10px] font-black text-white uppercase bg-slate-900 px-2 py-0.5 sm:px-3 sm:py-1 rounded-full border border-slate-700 shadow-xl">AV</span>
              </div>
            </div>
          </div>

          <div className="w-full grid grid-cols-2 gap-3 sm:gap-6 mt-6 sm:mt-12 relative z-50">
            <div className={`text-center p-3 sm:p-5 rounded-xl sm:rounded-2xl border transition-all duration-500 ${data.draftAft > data.draftForward ? 'bg-blue-600 border-white shadow-[0_0_20px_rgba(37,99,235,0.3)]' : 'bg-slate-900/95 border-slate-700'}`}>
              <p className="text-xl sm:text-4xl font-black text-white tracking-tighter">{data.draftAft.toFixed(2)}m</p>
              <p className={`text-[8px] sm:text-[10px] font-black uppercase tracking-widest mt-1 ${data.draftAft > data.draftForward ? 'text-white' : 'text-slate-500'}`}>CALADO AR</p>
            </div>
            <div className={`text-center p-3 sm:p-5 rounded-xl sm:rounded-2xl border transition-all duration-500 ${data.draftForward > data.draftAft ? 'bg-amber-600 border-white shadow-[0_0_20px_rgba(217,119,6,0.3)]' : 'bg-slate-900/95 border-slate-700'}`}>
              <p className="text-xl sm:text-4xl font-black text-white tracking-tighter">{data.draftForward.toFixed(2)}m</p>
              <p className={`text-[8px] sm:text-[10px] font-black uppercase tracking-widest mt-1 ${data.draftForward > data.draftAft ? 'text-white' : 'text-slate-500'}`}>CALADO AV</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-950/50 border-2 border-slate-800 rounded-[1.5rem] sm:rounded-[2.5rem] p-4 sm:p-8 flex flex-col items-center min-h-[350px] sm:min-h-[480px]">
          <div className="w-full flex justify-between items-center mb-4 sm:mb-8">
            <span className="text-[9px] sm:text-[11px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
              <Activity size={14} className="sm:w-4 sm:h-4" /> Banda Real (BB/BE)
            </span>
          </div>
          <div className="flex-1 flex items-center justify-center w-full">
            <div className="relative w-48 h-48 sm:w-72 sm:h-72 border-4 sm:border-8 border-slate-900 rounded-full flex items-center justify-center bg-slate-950/50 shadow-inner">
              <div className="absolute top-0 w-1 sm:w-1.5 h-8 sm:h-12 bg-red-600 rounded-full -translate-y-1/2 z-20" />
              <div 
              ...
              >
                <div className="absolute bottom-0 left-0 w-full h-8 sm:h-12 bg-slate-800 rounded-b-[1.5rem] sm:rounded-b-[3rem] border-b-[6px] sm:border-b-[10px] border-blue-600" />
                <div className="absolute top-0 left-0 w-full h-5 sm:h-8 bg-slate-700 rounded-t-sm sm:rounded-t-lg shadow-xl" />
              </div>
              <div className="absolute -bottom-4 sm:-bottom-6 bg-slate-900 border sm:border-2 border-blue-500/40 px-4 py-2 sm:px-8 sm:py-3 rounded-xl sm:rounded-2xl shadow-2xl">
                <span className="text-2xl sm:text-5xl font-black text-white tracking-tighter">{Math.abs(data.heel).toFixed(1)}°</span>
                <span className="text-[10px] sm:text-sm font-black text-blue-400 ml-2 sm:ml-3 uppercase">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6">
        ...
        <div className="bg-slate-800/40 border border-slate-700/40 rounded-xl sm:rounded-2xl p-4 sm:p-5 shadow-inner transition-all hover:bg-slate-800/60 group">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className="bg-slate-900 rounded-lg sm:rounded-xl text-amber-500 p-2 sm:p-2.5 border border-slate-800 group-hover:scale-110 transition-transform shrink-0">
                <Activity size={18} />
              </div>
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <span className="text-slate-500 font-black uppercase text-[10px] sm:text-xs">°</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-800/40 border border-slate-700/40 rounded-xl sm:rounded-2xl p-4 sm:p-5 shadow-inner transition-all hover:bg-slate-800/60 group">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className="bg-slate-900 rounded-lg sm:rounded-xl text-blue-500 p-2 sm:p-2.5 border border-slate-800 group-hover:scale-110 transition-transform shrink-0">
                <Activity size={18} />
              </div>
        ...
        <div className="bg-slate-800/40 border border-slate-700/40 rounded-xl sm:rounded-2xl p-4 sm:p-5 shadow-inner transition-all hover:bg-slate-800/60 group">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <span className="text-slate-500 font-black uppercase text-[10px] sm:text-xs">m</span>
            </div>
          </div>
        </div>
